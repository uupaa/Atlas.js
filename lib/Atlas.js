...
Atlas["prototype"]["draw"]   = Atlas_draw;   // Atlas#draw(key:String, ctx:CanvasRenderingContext2D, dx:UINT16, dy:UINT16, dw:UINT16, dh:UINT16):void
...
//{@dev
    if (!global["BENCHMARK"]) {
        ...
        $valid($type(dx,  "UINT16"),      Atlas_draw, "dx");
        $valid($type(dy,  "UINT16"),      Atlas_draw, "dy");
        $valid($type(dw,  "UINT16|omit"), Atlas_draw, "dw");
        $valid($type(dh,  "UINT16|omit"), Atlas_draw, "dh");
    }
//}@dev
